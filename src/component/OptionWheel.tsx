...
const OptionWheel = ({
  ...
  useEffect(() => {
    if (defaultSelected !== targetRef.current) {
      targetRef.current = defaultSelected;
      const idx = ((Math.round(defaultSelected) % cfgRef.current.count) + cfgRef.current.count) % cfgRef.current.count;
      selectedRef.current = idx;
      setSelectedIndex(idx);
      startLoop();
    }
  }, [defaultSelected, startLoop]);
      ...
      {items.map((label, index) => (
        ...
        >
          <div className="flex items-center gap-4">
            {side === 'left' && <div className={`h-[2px] bg-current rounded-full transition-all duration-300 ${selectedIndex === index ? 'w-12' : 'w-4 opacity-50'}`} />}
            <span>{label}</span>
            {side === 'right' && <div className={`h-[2px] bg-current rounded-full transition-all duration-300 ${selectedIndex === index ? 'w-12' : 'w-4 opacity-50'}`} />}
          </div>
        </div>
      ))}
    </div>
