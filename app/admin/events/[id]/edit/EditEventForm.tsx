...
import { updateEvent } from "@/src/actions/admin";
...
export default function EditEventForm({ event }: { event: any }) {
  ...
  const [existingGallery, setExistingGallery] = useState<string[]>(event.gallery || []);
  ...
  const [formData, setFormData] = useState({
    ...
    buttons: (event.buttons as { label: string, url: string }[]) || [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      setError("Failed to update event. Please check the details and try again.");
    ...
    <div>
      <div className="flex items-center gap-4 mb-8 border-b border-white/10 pb-4">
        ...
        <div>
          ...
          <p className="text-white/50 text-sm">Update the details for {event.title}.</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          {formData.date && new Date(formData.date) < new Date() && (
            <div className="flex flex-col gap-2">
              ...
              <div className="border-2 border-dashed border-white/20 rounded-xl p-8 flex flex-col items-center justify-center bg-white/5 hover:bg-white/10 transition-colors relative cursor-pointer min-h-[200px]">
                <input 
                  type="file" 
                  accept="image/*"
                  multiple
                ...
                {(existingGallery.length > 0 || galleryPreviews.length > 0) ? (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 w-full relative z-20">
                    {/* Existing Gallery Images */}
                    {existingGallery.map((url, i) => (
                      <div key={`existing-${i}`} className="relative aspect-square rounded overflow-hidden border border-white/20 group pointer-events-auto">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        ...
                        <button 
                          ...
                          className="absolute top-1 right-1 bg-black/80 backdrop-blur-md border border-white/10 text-white/70 hover:text-white rounded-full p-1.5 flex items-center justify-center transition-all z-30 hover:bg-red-500 hover:border-red-500 shadow-lg"
                          title="Remove image"
                        >
                          <X className="w-3 h-3" />
                        </button>
                    ...
                    {galleryPreviews.map((preview, i) => (
                      <div key={`new-${i}`} className="relative aspect-square rounded overflow-hidden border border-white/20 group pointer-events-auto">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                    ...
                    <div className="aspect-square rounded border border-dashed border-white/20 flex flex-col items-center justify-center text-white/30 hover:bg-white/5 hover:text-white/50 transition-colors cursor-pointer pointer-events-none">
